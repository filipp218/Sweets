...
from schema import CourierItem, CourierUpdateRequest, OrderItem, OrdersAssignPostRequest, OrdersCompletePostRequest
from sql import get_database, create_courier, profile, change_courier_type, change_regions_courier, change_working_hours_courier, create_order, create_order_complete, assign_order, delete_order_assign, check_order_complete, check_order_in_assign, change_current_time, add_time_delivery, profile_after_change, all_couriers_check, all_orders_check
from work_with_time import validate_work_hours, information_error_courier, information_error_order
...
def add_couriers():
    ...
    for row in json_couriers['data']:
        try:
            ...
            id_error["couriers"].append({"id":row["courier_id"], "message": information_error_courier(ex)})

    if len(id_error["couriers"])>0:
        return answer_error, 400

    with get_database() as db:
        all_couriers = all_couriers_check(db) #список id курьеров, которые уже есть, чтобы не добавлять курьера если id уже занят
        for row in json_couriers['data']:
            if row["courier_id"] in all_couriers:
                continue
            create_courier(db, row["courier_id"], row["courier_type"], row["working_hours"], row["regions"])
...
def add_orders():
    ...
    for row in json_orders['data']:
        try:
            ...
            id_error["orders"].append({"id":row["order_id"], "message":information_error_order(ex)})

    if len(id_error["orders"])>0:
        return answer_error, 400

    with get_database() as db:
        all_orders = all_orders_check(db)#список id заказов, которые уже есть, чтобы не добавлять заказ если id уже занят
        for row in json_orders['data']:
            if row["order_id"] in all_orders:
                continue
            create_order(db, row["order_id"], row["weight"], row["region"], row["delivery_hours"])
...
def orders_complete():
    ...
    with get_database() as db:
        ...
        if not check_order_in_assign(db, json_orders_complete["courier_id"], json_orders_complete["order_id"]):
            abort(400)
        create_order_complete(db, json_orders_complete["courier_id"], json_orders_complete["order_id"], json_orders_complete["complete_time"])
        delete_order_assign(db, json_orders_complete["order_id"])
        add_time_delivery(db, json_orders_complete["order_id"], json_orders_complete["courier_id"], json_orders_complete["complete_time"] )
        change_current_time(db, json_orders_complete["order_id"], json_orders_complete["courier_id"], json_orders_complete["complete_time"])
        db.commit()
    return {"order_id" : json_orders_complete["order_id"]}
